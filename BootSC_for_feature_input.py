...
class Model(LightningModule):
    def __init__(self, n_classes, max_epochs, input_dim=768):
        super().__init__()
        # Storing parameters
...
class EmbeddingPairDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            x0 = self.transform(x) if self.transform is not None else x
            x1 = self.transform(x) if self.transform is not None else x
            return (x0, x1), target, idx
        else:
            return x, target, idx
...
trainset = EmbeddingPairDataset(X, y, transform=feature_transform, train=True)
valset = EmbeddingPairDataset(X, y, transform=None, train=False)
...
input_dim = X.shape[1]
BootSC = Model(classes, max_epochs, input_dim=input_dim)
